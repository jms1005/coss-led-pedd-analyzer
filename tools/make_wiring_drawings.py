...
PINS = [
  ...
  ("PD7", "D7", "검출 LED 캐소드 (렌즈 안 큰 컵 쪽)", C_DET),
  ("PD2", "D2", "버튼 SW1", C_BTN),
  ("PC4", "A4", "OLED SDA", INK),
  ("PC5", "A5", "OLED SCL", INK),
  ("—", "5V", "OLED VCC", INK),
...
def wire1():
  ...
  ty += 5
  sh.band(MARGIN, ty, 96, 44, "다리 길이보다 **렌즈 안쪽**을 보세요",
          ["투명 렌즈 안을 들여다보면 한쪽에 **큰 컵**이",
           "들어 있습니다. 그쪽이 캐소드(−) 입니다.",
           "다리를 자르면 길이 단서는 사라지지만",
           "이 컵은 남습니다.",
           "",
           "· 애노드(+, 긴 다리) → 저항 쪽",
           "· 캐소드(−, 큰 컵)  → GND 쪽"], color=GREEN, lead=4.2)

  sh.warn(PW / 2.0 + 4, ty, PW / 2.0 - MARGIN - 4,
          "검출 LED 하나만 반대입니다",
          ["발광 LED 3개는 짧은 다리가 GND 로 갑니다. 보통 방식입니다.",
           "",
           "검출 LED 만 **캐소드(−)가 우노 D7 로**, 애노드(+)가 GND 로",
           "갑니다. 거꾸로 걸어야 빛을 받아 전기를 만드는 소자가 됩니다.",
           "",
           "**거꾸로 꽂으면 측정값이 0~1 tick 에 고정됩니다.** (실측 확인)"])
...
CHECKS = [
  ...
  ("청", "26 A", "위 − 줄", C_GND),
  ("검출", "우노 D7", "3 J", C_DET),
  ("검출", "검출 LED 캐소드(−, 렌즈 안 큰 컵 쪽)  ⚠", "3 G", C_DET),
  ("검출", "검출 LED 애노드(+)  ⚠", "5 G", C_DET),
  ("검출", "5 J", "아래 − 줄", C_GND),
...
def wire4():
  ...
  sh.warn(MARGIN, ty, PW - 2 * MARGIN, "USB 를 꽂기 직전에 이 세 가지",
          ["① 검출 LED 의 캐소드(렌즈 안 큰 컵 쪽)가 D7 인가 — 발광 3개와 반대입니다.",
           "② 5V 와 GND 가 어디에서도 직접 맞닿아 있지 않은가 (합선).",
           "③ 부품 다리끼리 서로 닿아 있지 않은가. 특히 LED 두 다리."])
  ty += 12 + 3 * 4.4 + 6

  sh.band(MARGIN, ty, PW - 2 * MARGIN, 44, "잘 안 될 때 어디부터 보나 (실측 기준)",
          ["· 값이 **0~1 tick 고정**  →  검출 LED 방향. 그대로 180° 돌리세요.",
           "· **전 채널 TIMEOUT**  →  검출 LED 미접속·단선. 다리와 점퍼를 다시 꽂기.",
