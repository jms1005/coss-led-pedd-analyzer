...
CHECKS = [
  ...
  ("검출", "검출 LED 짧은 다리(−)  ⚠", "3 G", W.C_DET),
  ("검출", "검출 LED 긴 다리(+)  ⚠", "5 G", W.C_DET),
  ("검출", "5 J", "아래 − 줄", W.C_GND),
]
...
def sheet():
  ...
  sh.warn(MARGIN, 230, PW - 2 * MARGIN,
          "검출 LED — 이 회로에서 유일하게 거꾸로 꽂는 부품",
          ["짧은 다리(−) → 우노 D7,  긴 다리(+) → GND 입니다. 바로 꽂으면",
           "순방향이 되어 측정 원리가 성립하지 않습니다. 저항은 넣지 않습니다.",
           "색은 반드시 **적색**. 청·녹은 Red 채널이 전부 TIMEOUT 으로 찍힙니다."])

  sh.band(MARGIN, 259, PW - 2 * MARGIN, 20, "다 꽂은 뒤",
          ["11칸을 모두 확인한 다음에 USB 를 꽂습니다. 배선 중 전원이 들어가",
           "있으면 잘못 닿았을 때 우노가 상합니다.  수신 설정: **38400 bps, 8N1**."],
          color=GREEN)
