 ...
 * ---- 처음 읽는 사람을 위한 요약 ----
 *
 * LED 는 빛을 내기도 하지만, 빛을 받으면 아주 약한 전기를 만들기도 한다.
 * 이 파일은 그 성질로 밝기를 재는데, 밝기를 직접 재지 않고 "시간"을 잰다.
 *
 * 물컵으로 비유하면 이렇다.
 *
 *   1. 검출 LED 에 전기를 가득 채운다        (충전, 0.0001초)
 *   2. 채우던 관을 잠근다                    (핀을 입력으로 전환)
 *   3. 빛이 들어오면 전기가 새기 시작한다    (빛 = 컵에 뚫린 구멍)
 *   4. 다 새기까지 걸린 시간을 잰다          (밝을수록 짧다)
 *
 * 그래서 반환값 ticks 는 밝기에 반비례한다. 빛이 셀수록 작아진다.
 * 이걸 밝기 비율로 바꾸는 일은 classify.c 가 맡는다.
 *
 * 시간을 재는 데 소프트웨어 대신 하드웨어를 쓰는 것이 이 파일의 핵심이다.
 * "전압이 다 떨어졌나?" 를 코드로 계속 물어보면(폴링) 묻는 간격만큼 오차가
 * 생긴다. 대신 칩 안의 비교기와 타이머를 서로 연결해 두면, 전압이 기준선을
 * 지나는 순간 하드웨어가 알아서 시각을 기록한다. 코드는 나중에 그 값을
 * 읽기만 하면 되므로 소프트웨어 지연이 측정값에 섞이지 않는다.
 *
 * 아래 "측정 원리" 는 같은 내용을 전자공학 용어로 다시 쓴 것이다.
 *
 * 측정 원리 (계획서 3.2절)
